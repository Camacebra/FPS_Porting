...
public class GameSystem : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        SaveData.currentData = (SaveData)SaveSystem.LoadData("Save1");
        if (SaveData.currentData != null){
            m_Score = SaveData.currentData.data.Score;
            playedTimes = SaveData.currentData.data.PlayedTimes;
            playTime = SaveData.currentData.data.PlayTime;
            Debug.Log("Datos caragados");
            Debug.Log("Score: " + m_Score);
            Debug.Log("Times played: " + playedTimes);
            Debug.Log("Play Time: " + playTime);
            Debug.Log("And Ammos");
        }
        playedTimes++;
        SaveData.currentData.data.PlayedTimes++;
    ...
    public void TargetDestroyed(int score)
    {
        m_TargetDestroyed += 1;
        m_Score += score;
        SaveData.currentData.data.Score = m_Score;
        GameSystemInfo.Instance.UpdateScore(m_Score);
    }
}
