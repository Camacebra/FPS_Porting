...
public class Controller : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            if (SaveData.currentData != null && SaveData.currentData.Ammo !=null)
            {
                startingAmmo[i].amount = SaveData.currentData.Ammo[i].Amount;
            }
        }
        for (int i = 0; i < startingAmmo.Length; ++i)
        {
    ...
    void Update()
    {
        ...
        if (!m_IsPaused && !LockControl)
        {
            ...
            if (m_Grounded && NewInputs.instace.JumpDown)
            {
            ...
            bool running = m_Weapons[m_CurrentWeapon].CurrentState == Weapon.WeaponState.Idle && NewInputs.instace.RunPressed;
            ...
            move = NewInputs.instace.GetMovementDirection;
            ...
            // Turn player
            float turnPlayer = NewInputs.instace.CameraDirection.x * MouseSensitivity;
            ...
            // Camera look up/down
            var turnCam = -NewInputs.instace.CameraDirection.y;
            turnCam = turnCam * MouseSensitivity;
            m_VerticalAngle = Mathf.Clamp(turnCam + m_VerticalAngle, -89.0f, 89.0f);
            currentAngles = CameraPosition.transform.localEulerAngles;
            currentAngles.x = m_VerticalAngle;
            CameraPosition.transform.localEulerAngles = currentAngles;
            m_Weapons[m_CurrentWeapon].triggerDown = NewInputs.instace.ShootPressed;
            Speed = move.magnitude / (PlayerSpeed * Time.deltaTime);

            if (NewInputs.instace.ReloadPressed)
                m_Weapons[m_CurrentWeapon].Reload();

            if (NewInputs.instace.SwitchWeapon < 0)
            {
                ChangeWeapon(m_CurrentWeapon - 1);
            }
            else if (NewInputs.instace.SwitchWeapon > 0)
            {
                ChangeWeapon(m_CurrentWeapon + 1);
            }

            //Key input to change weapon
            if (NewInputs.instace.SelectWeapon != 0)
            {
                int i = (int)NewInputs.instace.SelectWeapon;
                int num = 0;
                if (i == 0)
                    num = 10;
                else
    ...
    void SaveDatas()
    {
        ...
        for (int i = 0; i < startingAmmo.Length; i++){
            ...
            newAmmo.Amount = m_AmmoInventory[startingAmmo[i].ammoType];
            SaveData.currentData.Ammo.Add(newAmmo);
        }
        //saving playTime
        SaveData.currentData.data.PlayTime += Time.time;
        Debug.Log(SaveData.currentData.data.PlayTime);
        //saving data in archive save1
        SaveSystem.SaveData("Save1", SaveData.currentData);
    }
